...
public class SparqlUI extends JDialog implements HyperlinkListener {
  ...
  private JCheckBox indexWithClassCheckBox;
  private JCheckBox searchWithClassCheckBox;
...
//          FileInputStream slangIs = new FileInputStream(new File(owlFileTxt.getText()));
        ...
        try {
          ...
          File[] files = folder.listFiles(new FilenameFilter() {
            ...
            public boolean accept(File dir, String name) {
              return name.toLowerCase().endsWith(".txt")
                  && !name.toLowerCase().endsWith(SparqlQuery.CLASS_SUFFIX); //ignoremos clase
            }
          });
          ...
          if (files == null || files.length == 0) {
            ...
            for (File file : files) {
              ...
              if (indexWithClassCheckBox.isSelected()) {
                File classFile = new File(file.getAbsolutePath() + SparqlQuery.CLASS_SUFFIX);
                byte[] classRaw = Files.readAllBytes(classFile.toPath());
                indexer.indexar(file.toPath().toString(), new String(raw, Charsets.UTF_8), new String(classRaw, Charsets.UTF_8));
                sb.append("<p><a href=\"" + file.toURI().toURL() + "\">" + file.getName() + "</a> and <a href=\"" + classFile.toURI().toURL() + "\">class</a> indexed..." + "</p>");
              } else {
                indexer.indexar(file.toPath().toString(), new String(raw, Charsets.UTF_8));
                sb.append("<p><a href=\"" + file.toURI().toURL() + "\">" + file.getName() + "</a> indexed..." + "</p>");
              }

            }
            numberOfFiles = files.length;
          }
          sb.append("<p><b>" + numberOfFiles + " indexed files</b></p>");

          luceneEditorPane.setText(sb.toString());
          showInfo(numberOfFiles + " documentos");
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        try {
          ...
          for (int i = 0; i < tokens.length; i++) {
            if (i != 0 && !tokens[i].isEmpty()) {  // ignore header and lastline
              words += tokens[i];
    ...
    searchBtn.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent evt) {
        ...
        try {
          ...
          StringBuilder sb = new StringBuilder("<p align=\"center\"><b>PERUVIAN SLANG</b></p>");
          int numberOfFiles = 0;
          for (String word : words) {
            if (!word.isEmpty()) {
              sb.append("<p><b>Results for \"" + word + "\":</b></p>");
              ScoreDoc[] docs;
              if (searchWithClassCheckBox.isSelected()) {
                docs = searcher.buscarPorClase(word);
              } else {
                docs = searcher.buscar(word);
              }
              sb.append(searcher.visualizarDocumentos2(docs));
              numberOfFiles += docs.length;
            }
          }

          luceneEditorPane.setText(sb.toString());
          showInfo(numberOfFiles + " documentos");
          //searcher.cerrarBuscador(); //closed in main
  ...
  public static void main(String[] args) throws ClassNotFoundException, UnsupportedLookAndFeelException, InstantiationException, IllegalAccessException, IOException {
    ...
    if (Searcher.getInstance(null) != null)
      Searcher.getInstance(null).cerrarBuscador();
    System.exit(0);
  }
  ...
  public void hyperlinkUpdate(HyperlinkEvent e) {
    if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
      if (Desktop.isDesktopSupported()) {
        try {
//          System.out.println("URI: "+e.getURL().toURI());
//          System.out.println("Path: "+ Paths.get(e.getURL().toURI()));
          Desktop.getDesktop().browse(e.getURL().toURI());
//          Desktop.getDesktop().browse(Paths.get(e.getURL().toURI()).toUri());
        } catch (IOException e1) {
          e1.printStackTrace();
        } catch (URISyntaxException e1) {
          e1.printStackTrace();
